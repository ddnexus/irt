...
'add_test' (or 'at') and eventually add a description, and the testing code will be automatically
generated and added to the history.
...
IRT can compare complex objects and shows the diffs. When a test fails IRT will also show you a very
readable yaml dump with the differences between the expected and actual values, so you can immediately
find even a little problem inside a very complex object.
...
The default ~/.irtrc file contain the following:
    ...
    require 'irt'
    require 'irt/init'
    IRT.autoload_helper_files

==== Upgrade

Upgrading from one version to another requires that you manually delete the auto-generated ~/.irtrc file.
...
You can also pass a few configuration variables in the ~/.irtrc file:
    ...
    IRT.history.move_desc = false # don't move the description at the beginning of the block
    IRT.differ.classic = true # don't use diff reports

=== Note about IRT.autoload_helper_files

This method is called from the default ~/.irtrc file. It will require all the 'irt_helper.rb' named files in
the descending path fom the current working dir (which is considered the test-root dir) to the dir containing
the irt file being executed. That is useful to add special methods or overridig to your test files without
...
will not be loaded, so be carefull to be in the right dir to make it work properly.
